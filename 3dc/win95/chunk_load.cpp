...
#include "environment_chunk.hpp"
#include "chunk_object.hpp"
#include "chunk_palette.hpp"
#include "bitmap_names.hpp"
#include "chunk_light.hpp"
#include "chunk_textures.h"
#include "chunk_sprites.hpp"
#include "chunk_gsprites.hpp"
#include "animation_chunk.hpp"
#include "chunk_fragments.hpp"
#include "jsndsup.h"
#include "mempool.h"
#include <math.h>
// for log file
...
Shape_Fragment_Type::~Shape_Fragment_Type()
{
	...
	if (sh_fragdesc)
	{
		...
		if (sh_fragdesc->sh_frags)
		{
			DeallocateMem(sh_fragdesc->sh_frags);
		}
...
#define SIM_HASH_MASK (SIM_HASH_SIZE-1)
	...
	public:
		...
		inline int Listpos() const
		{
			return listpos;
		}
		inline SHAPEHEADER *Shptr() const
		{
			return shptr;
		}
		...
		inline BOOL operator != (ShapeInMSL const &s2) const
		{
			return ! operator == (s2);
		}
};
...
void ShapeInMSL::PurgeMSLShapeList()
{
	...
	{
		while (hash_msl[i].size())
		{
			hash_msl[i].delete_first_entry();
		}

		while (hash_ptr[i].size())
		{
			hash_ptr[i].delete_first_entry();
		}

		while (hash_name[i].size())
		{
			hash_name[i].delete_first_entry();
		}
	}
...
CTM_ReturnType copy_to_mainshapelist(RIFFHANDLE h, Shape_Chunk *tmpshp, int flags, const ChunkObject *object)
{
	...
	**-------------------*/
	...
	if (local_tex_index_nos != h->tex_index_nos)
	{
		delete[] local_tex_index_nos;
	}
...
BOOL load_rif_bitmaps(RIFFHANDLE h, int /*flags*/)
{
	...
	if (h->envd)
	{
		...
		if (pChunk)
		{
			gbnc = (Global_BMP_Name_Chunk *) pChunk;
		}
	}
		...
		if (shp->shapeflags & ShapeFlag_MultiViewSprite)
		{
			...
			for (int j = 1; thlist[j] != 0; j++)
			{
				...
				for (k = 0; k < th->txa_numframes; k++)
				{
					...
					if (tf->txf_uvdata[0])
					{
						DeallocateMem(tf->txf_uvdata[0]);
					}

					if (tf->txf_uvdata)
					{
						DeallocateMem(tf->txf_uvdata);
					}

					if (tf->txf_images)
					{
						DeallocateMem(tf->txf_images);
					}
				}

				if (th->txa_framedata)
				{
					DeallocateMem(th->txa_framedata);
				}
				...
				if (is_textured(shp->items[i][0]))
				{
					...
					if (shp->items[i][2]& iflag_txanim)
					{
						...
						for (j = 1; thlist[j] != 0; j++)
						{
							...
							for (k = 0; k < th->txa_numframes; k++)
							{
								if (th->txa_framedata[k].txf_uvdata)
								{
									DeallocateMem(th->txa_framedata[k].txf_uvdata);
								}
							}
					...
					{
						if (shp->sh_textures[UVIndex])
						{
							DeallocateMem(shp->sh_textures[UVIndex]);
						}
					}
				}
			}
	...
	if (shp->animation_header)
	{
		...
		if (shp->animation_header->num_shapes_using_this == 0)
		{
			...
			for (int i = 0; i < sah->num_sequences; i++)
			{
				...
				if (sas->vertex_normals)
				{
					DeallocateMem(sas->vertex_normals);
				}

				if (sas->anim_frames)
				{
					DeallocateMem(sas->anim_frames);
				}
			}
...
void copy_to_module(Object_Chunk *ob, int mod_pos, int shplst_pos)
{
	...
	*((int *)MainScene.sm_module[mod_pos].m_name) = mod_pos + ONE_FIXED;
	...
	if (opdc)
	{
		...
		if (pChunk)
		{
			mapblok = (Map_Block_Chunk *)pChunk;
		}
...
void SetupAnimOnQuad(Shape_Chunk *sc, SHAPEHEADER *shp, TEXANIM *ta1, TEXANIM *ta2, int poly, int *local_tex_index_nos)
{
	...
	for (i = 0; i < ta1->NumSeq; i++)
	{
		...
		if (!(ta1->AnimFlags & AnimFlag_NotPlaying))
		{
			th->txa_flags |= txa_flag_play;
		}
	...
	if (smdc)
	{
		...
		for (int i = 0; i < ac->NumPolys; i++)
		{
			...
			else if (mgd[ta1->poly] > ta1->poly)
			{
				...
				{
					if (ac->AnimList[j]->poly == mgd[ta1->poly])
					{
						break;
					}
				}
...
void SetupAnimatingShape(Shape_Chunk *sc, SHAPEHEADER *shp, Shape_Merge_Data_Chunk *smdc)
{
	...
	for (chlif.restart(); !chlif.done(); chlif.next())
	{
		...
		//      double square_root = x*x+y*y+z*z;
	...
	for (unsigned int i = 0; i < shp->animation_header->num_sequences; i++)
	{
...
)
{
	...
	*/
	...
	if (pchAnim)
	{
		...
		if (pCh)
		{
			smdc = (Shape_Merge_Data_Chunk *)pCh;
		}
	...
	for (int i = 0; i < shp.num_polys; i++)
	{
		...
		else if (mgd[i] > i)
		{
			...
			for (j = 0; j < 3; j++)
			{
				for (k = 0; k < 3; k++)
				{
					if (shp.poly_list[mpoly].vert_ind[j] == shp.poly_list[i].vert_ind[k])
					{
						break;
					}
				}
