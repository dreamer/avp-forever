...
void* InitVideoScreen(void* bhdata,STRATEGYBLOCK *sbPtr)
{
	...
	videoScreen = static_cast<VIDEO_SCREEN_BEHAV_BLOCK*>(AllocateMem(sizeof(VIDEO_SCREEN_BEHAV_BLOCK)));
	if(!videoScreen)
	{
...
void VideoScreenBehaviour(STRATEGYBLOCK *sbPtr)
{
	...
	VIDEO_SCREEN_BEHAV_BLOCK* videoScreen;
	GLOBALASSERT(sbPtr);
	videoScreen = static_cast<VIDEO_SCREEN_BEHAV_BLOCK*>(sbPtr->SBdataptr);
