...
void CreateMarineBot(VECTORCH *Position, int weapon)
{
	...
	if(sbPtr->SBdataptr)
	{
		...
		if ( (weapon<=0)||(weapon>(int)MNPCW_End)) {
			...
			#else
			marineStatus->My_Weapon = GetThisNPCMarineWeapon(static_cast<enum marine_npc_weapons>(weapon-1));
			#endif
		}
...
static void ProcessFarMarineTargetModule(STRATEGYBLOCK *sbPtr, AIMODULE* targetModule)
{
	...
	targetStatus = GetTargetAIModuleStatus(sbPtr, targetModule, 0); // added 3rd parameter. assume its correct?
	switch(targetStatus)
	{
...
void LoadStrategy_Marine(SAVE_BLOCK_STRATEGY_HEADER* header)
{
	...
	else
	{
		...
		tdm.marine_type = static_cast<enum marine_npc_weapons>(block->weapon_id);
	...
	//get marine's weapon
	marineStatusPointer->My_Weapon = GetThisNPCMarineWeapon(static_cast<enum marine_npc_weapons>(block->weapon_id));
...
void SaveStrategy_Marine(STRATEGYBLOCK* sbPtr)
{
	...
	GET_STRATEGY_SAVE_BLOCK(MARINE_SAVE_BLOCK,block,sbPtr);
	marineStatusPointer = (MARINE_STATUS_BLOCK*) sbPtr->SBdataptr;
...
void SaveMarineSquadState()
{
	...
	GET_SAVE_BLOCK_POINTER(MARINE_SQUAD_SAVE_BLOCK, block);
