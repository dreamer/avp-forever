...
static void Execute_PAQFS_Hunt(STRATEGYBLOCK *sbPtr)
{
	...
	/* get the target */
	targetModule = FarNPC_GetTargetAIModuleForHunt(sbPtr, /* bjd - pass 0 here?? */ 0);
	if(!targetModule)
	{
		paqStatusPointer->stateTimer = PAQ_FAR_MOVE_TIME;
		return;
...
static void Execute_PAQFS_Wander(STRATEGYBLOCK *sbPtr)
{
	...
	/* get the target */
	targetModule = FarNPC_GetTargetAIModuleForWander(sbPtr, NULL, /* bjd - pass 0 here?? */ 0);
...
static void ProcessFarPAQTargetModule(STRATEGYBLOCK *sbPtr, AIMODULE* targetModule)
{
	...
	targetStatus = GetTargetAIModuleStatus(sbPtr, targetModule, 0); // bjd - check 3rd parameter '0' as I added this. wasn't here before
	switch(targetStatus)
	{
...
static void Execute_PAQNS_Approach(STRATEGYBLOCK *sbPtr)
{
	...
	/* Do some approach sound */
  {
	...
	NPCGetMovementTarget(sbPtr, Player->ObStrategyBlock, &targetPosition, &targetIsAirduct, /* bjd - pass 0 here?? */ 0);

	// bjd - not sure what to do here, pass a temp WAYPOINT_MANAGER pointer?
	WAYPOINT_MANAGER temp;

//	NPCGetMovementDirection(sbPtr, &velocityDirection, &targetPosition, 1);
	NPCGetMovementDirection(sbPtr, &velocityDirection, &targetPosition, &temp);
...
//bjd			CauseDamageToObject(destructableObject, &TemplateAmmo[AMMO_NPC_OBSTACLE_CLEAR].MaxDamage, ONE_FIXED,NULL);
...
//bjd		CauseDamageToObject(Player->ObStrategyBlock,&TemplateAmmo[AMMO_NPC_PAQ_CLAW].MaxDamage, ONE_FIXED,NULL);
		CauseDamageToObject(Player->ObStrategyBlock,&TemplateAmmo[AMMO_NPC_PAQ_CLAW].MaxDamage[AvP.Difficulty], ONE_FIXED,NULL);
...
static void Execute_PAQNS_Wander(STRATEGYBLOCK *sbPtr)
{
	...
	{
		NPC_InitMovementData(&(paqStatusPointer->moveData));
		NPC_FindAIWanderTarget(sbPtr,&(paqStatusPointer->wanderData),&(paqStatusPointer->moveData), /* bjd - pass 0 here?? */ 0);
	}
...
//	NPCGetMovementDirection(sbPtr, &velocityDirection, &(paqStatusPointer->wanderData.worldPosition),1);
	NPCGetMovementDirection(sbPtr, &velocityDirection, &(paqStatusPointer->wanderData.worldPosition), &temp);
