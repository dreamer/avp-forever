...
void SaveGame()
{
	...
	// bjd - check cast ok
	packagePtr = HuffmanCompression(reinterpret_cast<unsigned char*>(SaveInfo.BufferStart+headerLength), SaveInfo.BufferSpaceUsed-headerLength);
...
void LoadSavedGame()
{
	...
	AvP.Difficulty = static_cast<enum gamedifficulty>(save_slot->Difficulty);
	RestartLevel();
...
{
	MISC_GLOBAL_SAVE_BLOCK* block;
	block = static_cast<MISC_GLOBAL_SAVE_BLOCK*>(GetPointerForSaveBlock(sizeof(*block)));
