...
int FrisbeeSight_FrustumReject(STRATEGYBLOCK *sbPtr,VECTORCH *localOffset,STRATEGYBLOCK *target) {
...
static STRATEGYBLOCK* InitialiseFrisbeeBehaviour_ForLoad() {
	...
	bblk = static_cast<FRISBEE_BEHAV_BLOCK*>(dispPtr->ObStrategyBlock->SBdataptr);
...
STRATEGYBLOCK* CreateFrisbeeKernel(VECTORCH *position, MATRIXCH *orient, int fromplayer) {
	...
	if(AvP.Network != I_No_Network)	/*AddNetGameObjectID*/AssignNewSBName(dispPtr->ObStrategyBlock);
			...
			if (reportPtr->ObstacleSBPtr) 
			{
				...
				if (AccuracyStats_TargetFilter(reportPtr->ObstacleSBPtr)) 
				{
					if (bbPtr->player)
					{
						...
						if (slot!=-1) 
						{
							CurrentGameStats_WeaponHit(static_cast<enum WEAPON_SLOT>(slot), 1);
						}
					}
				}
...
extern void GrenadeBehaviour(STRATEGYBLOCK *sbPtr)
{
	...
	while (reportPtr)
	{
		...
		if(sbPtr)
		{
			...
			{
				NETGHOSTDATABLOCK *ghostData = static_cast<NETGHOSTDATABLOCK*>(sbPtr->SBdataptr);
...
void InitialiseDiscBehaviour(STRATEGYBLOCK *target,SECTION_DATA *disc_section) {
	...
	if (AvP.Network != I_No_Network)	
	{
		/*AddNetGameObjectID*/AssignNewSBName(dispPtr->ObStrategyBlock);
	}
}
...
static STRATEGYBLOCK* InitialiseDiscBehaviour_ForLoad() {
	...
	/* give missile a maximum lifetime */
	dispPtr->ObStrategyBlock->SBdataptr = AllocateMem(sizeof(PC_PRED_DISC_BEHAV_BLOCK));
	bblk = static_cast<PC_PRED_DISC_BEHAV_BLOCK*>(dispPtr->ObStrategyBlock->SBdataptr);
			...
			if (bbPtr->Stuck) {
				...
				if (SBForcesBounce(reportPtr->ObstacleSBPtr)) {
					...
					*/
					...
					case I_BehaviourNetGhost:
						{
							NETGHOSTDATABLOCK *dataptr = static_cast<NETGHOSTDATABLOCK*>(reportPtr->ObstacleSBPtr->SBdataptr);
							switch (dataptr->type) {
								case I_BehaviourMarinePlayer:
								case I_BehaviourAlienPlayer:
								case I_BehaviourPredatorPlayer:
...
void LoadStrategy_SpearBolt(/*SAVE_BLOCK_HEADER*/SAVE_BLOCK_STRATEGY_HEADER* header)
{
	DISPLAYBLOCK* dPtr;
	STRATEGYBLOCK* sbPtr;
