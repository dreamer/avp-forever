...
extern void KillAlienAIGhost(STRATEGYBLOCK *sbPtr,int death_code,int death_time,int GibbFactor) {
	...
	*/
	ghostData->IOType = static_cast<enum inanimateobject_type>(ghostData->subtype);
	...
	/* Now let's do the sequence. */
 	UpdateAlienAIGhostAnimSequence(sbPtr, static_cast<HMODEL_SEQUENCE_TYPES>(this_death->Sequence_Type), this_death->Sub_Sequence,
...
void UpdateAlienAIGhost(STRATEGYBLOCK *sbPtr,VECTORCH *position,EULER *orientation,int sequence_type,int sub_sequence, int sequence_length)
{
	...
	if ((sequence_type!=-1)&&(sub_sequence!=-1)) {
		/* Not tweening! */
		UpdateAlienAIGhostAnimSequence(sbPtr, static_cast<HMODEL_SEQUENCE_TYPES>(sequence_type),sub_sequence,sequence_length,(ONE_FIXED>>2));
	}
