...
// bjd - this is called when drawing the player's weapon
...
int Prune_Recursion_Virtual(SECTION_DATA *this_section_data)
{
	...
	if (this_section->flags & section_has_sparkoflife)
	{
		sol = 1;
	}
...
int Prune_HModel_Virtual(SECTION_DATA *top_section)
{
	...
	if (Prune_Recursion_Virtual(top_section))
	{
		sol = 1;
	}
...
#define SAVELOAD_BEHAV controller
	...
	void SaveHierarchy(HMODELCONTROLLER *controller)
	{
		...
		block->root_section_id = controller->Root_Section->IDnumber;
		{
			...
			//allocate memory, and copy names;
			buffer = static_cast<char *>(GetPointerForSaveBlock(strlen(Hierarchy_Name) + 1));
			strcpy(buffer, Hierarchy_Name);
			buffer = static_cast<char *>(GetPointerForSaveBlock(strlen(Rif_Name) + 1));
