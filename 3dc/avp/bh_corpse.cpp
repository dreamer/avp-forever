...
void Convert_Alien_To_Corpse(STRATEGYBLOCK *sbPtr,DEATH_DATA *this_death,DAMAGE_PROFILE* damage) {
	...
	sbPtr->I_SBtype=I_BehaviourNetCorpse;
 	...
 	SetCorpseAnimSequence_Core(sbPtr, static_cast<enum HModelSequenceTypes>(this_death->Sequence_Type), this_death->Sub_Sequence,
...
void LoadStrategy_Corpse(SAVE_BLOCK_STRATEGY_HEADER* header)
{
	...
	*sbPtr->DynPtr = block->dynamics;
	...
	corpseDataPtr->My_Weapon = GetThisNPCMarineWeapon(static_cast<enum marine_npc_weapons >(block->weapon_id));
