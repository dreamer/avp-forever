		...
		if (parent_sbPtr)
		{
			...
			switch (parent_sbPtr->I_SBtype) {
				...
				case I_BehaviourNetGhost:
					{
						NETGHOSTDATABLOCK *dataptr = static_cast<NETGHOSTDATABLOCK*>(parent_sbPtr->SBdataptr);
						switch (dataptr->type) {
							case I_BehaviourAlien:
							{
								DoAlienLimbLossSound(positionPtr);
...
void MakeFleshRippingNoises(VECTORCH *positionPtr)
{
	...
	Sound_Play(static_cast<enum soundindex>(SID_BODY_BEING_HACKED_UP_0+s),"d",positionPtr);
}
...
void LoadStrategy_Debris(/*SAVE_BLOCK_HEADER*/SAVE_BLOCK_STRATEGY_HEADER* header)
{
	STRATEGYBLOCK* sbPtr;
