...
void MaintainPlayersInventory(void)
{
	...
	while(nextReport)
	{
		...
		{
			INANIMATEOBJECT_STATUSBLOCK* objStatPtr = static_cast<INANIMATEOBJECT_STATUSBLOCK*>(collidedWith->SBdataptr);
			...
			if(collidedWith->SBflags.please_destroy_me==0 && objStatPtr->respawnTimer==0)
			{
				...
				switch(objStatPtr->typeId)
				{
					...
					case(IOT_Ammo):
					{
						if (AbleToPickupAmmo(static_cast<enum AMMO_ID>(objStatPtr->subType)))
						{
							RemovePickedUpObject(collidedWith);
...
//						SetPlayerSecurityClearance(Player->ObStrategyBlock,objStatPtr->subType);						
		...
		{
			NETGHOSTDATABLOCK* ghostData = static_cast<NETGHOSTDATABLOCK*>(collidedWith->SBdataptr);
			...
			if (collidedWith->SBflags.please_destroy_me==0) 
			{
				if (ghostData->type==I_BehaviourInanimateObject) 
				{
					...
					{
						if (AbleToPickupAmmo(static_cast<enum AMMO_ID>(ghostData->subtype)))
						{
					...
					{
						if (AbleToPickupWeapon(static_cast<enum WEAPON_ID>(ghostData->subtype)))
						{
							AddNetMsg_LocalObjectDestroyed_Request(collidedWith);
							ghostData->IOType=IOT_Non;
							/* So it's not picked up again... */
...
void Convert_Disc_To_Pickup(STRATEGYBLOCK *sbPtr) {
	...
	objectStatusPtr = static_cast<INANIMATEOBJECT_STATUSBLOCK*>(AllocateMem(sizeof(INANIMATEOBJECT_STATUSBLOCK)));
	...
	if ((weaponPtr->PrimaryMagazinesRemaining==0)
		...
		if (nearest) {
			...
			if (nearest->I_SBtype == I_BehaviourInanimateObject) {
				...
				INANIMATEOBJECT_STATUSBLOCK* objStatPtr = static_cast<INANIMATEOBJECT_STATUSBLOCK*>(nearest->SBdataptr);
			...
			{
				NETGHOSTDATABLOCK* ghostData = static_cast<NETGHOSTDATABLOCK*>(nearest->SBdataptr);
