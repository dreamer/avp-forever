...
D3DTLVERTEX *mainVertex = new D3DTLVERTEX[MAX_VERTEXES];
D3DTLVERTEX *tempVertex = new D3DTLVERTEX[MAX_VERTEXES];
...
RENDER_STATES *renderList = new RENDER_STATES[1000];
...
#define OUTPUT_TRIANGLE(a,b,c,n) \
	mainVertex[NumVertices] = tempVertex[a]; \
	NumVertices++; \
	mainVertex[NumVertices] = tempVertex[b]; \
	NumVertices++; \
	mainVertex[NumVertices] = tempVertex[c]; \
	NumVertices++;
...
//	if (NumVertices + real_num_verts > (MAX_VERTEXES-12))
	if (real_num_verts > (MAX_VERTEXES - NumVertices))
	{
...
//		renderList[renderCount].index_start = NumIndicies;
	...
	renderList[renderCount].vert_end = NumVertices + real_num_verts;
...
*/
	...
	for (unsigned int i = 0; i < renderCount; i++)
	{
		...
		int start = renderList[i].vert_start;
		int end = renderList[i].vert_end;
		...
		if (num_prims > 0)
		{
			...
			int totalDwordCount = (num_prims * 3) * 8;
			int vertIndex = start;
		...
		//		OutputDebugString(buf);
				...
				memcpy(pPush, (DWORD*) &mainVertex[vertIndex], (dwordCount / 8) * sizeof(D3DTLVERTEX));
...
//		char buf[100];
//		sprintf(buf, "%d verts, %d triangles\n", end-start, (end-start) / 3);
//		OutputDebugString(buf);
...
//	d3d.lpD3DDevice->SetVertexShader( D3DTLVERTEX );
	...
	memcpy(pPush, (DWORD*) &mainVertex, NumVertices * sizeof(D3DTLVERTEX));
/*
...
*/
	...
	tempVertex[0].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[0].tu = 0.0f;
	tempVertex[0].tv = (1.0f / real_height) * height;
...
//	vb++;
	...
	tempVertex[2].color = colour;
	tempVertex[2].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[2].tu = (1.0f / real_width) * width;
	tempVertex[2].tv = (1.0f / real_height) * height;
...
//	vb++;
	...
	tempVertex[3].color = colour;
	tempVertex[3].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[3].tu = (1.0f / real_width) * width;
	tempVertex[3].tv = 0.0f;
...
*/
	...
	// bottom left
	tempVertex[0].sx = (float)topX - 0.5f;
	tempVertex[0].sy = (float)topY + font_height - 0.5f;
	tempVertex[0].sz = 0.0f;
	tempVertex[0].rhw = 1.0f;
	tempVertex[0].color = colour;
	tempVertex[0].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[0].tu = (float)((texU) * RecipW);
	tempVertex[0].tv = (float)((texV + font_height) * RecipH);
...
//	vb++;
	...
	tempVertex[1].color = colour;
	tempVertex[1].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[1].tu = (float)((texU) * RecipW);
	tempVertex[1].tv = (float)((texV) * RecipH);

//	vb++;

	// bottom right
	tempVertex[2].sx = (float)topX + font_width - 0.5f;
	tempVertex[2].sy = (float)topY + font_height - 0.5f;
	tempVertex[2].sz = 0.0f;
	tempVertex[2].rhw = 1.0f;
	tempVertex[2].color = colour;
	tempVertex[2].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[2].tu = (float)((texU + font_height) * RecipW);
	tempVertex[2].tv = (float)((texV + font_height) * RecipH);

//	vb++;

	// top right
	tempVertex[3].sx = (float)topX + font_width - 0.5f;
	tempVertex[3].sy = (float)topY - 0.5f;
	tempVertex[3].sz = 0.0f;
	tempVertex[3].rhw = 1.0f;
	tempVertex[3].color = colour;
	tempVertex[3].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[3].tu = (float)((texU + font_width) * RecipW);
	tempVertex[3].tv = (float)((texV) * RecipH);
...
//	currentTextureId = -1;
	...
	// bottom left
	tempVertex[0].sx = (float)topX - 0.5f;
	tempVertex[0].sy = (float)topY + height_of_char - 0.5f;
	tempVertex[0].sz = 0.0f;
	tempVertex[0].rhw = 1.0f;
	tempVertex[0].color = colour;
	tempVertex[0].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[0].tu = (float)((texU) * RecipW);
	tempVertex[0].tv = (float)((texV + height_of_char) * RecipH);
...
//	vb++;
	...
	// bottom right
	tempVertex[2].sx = (float)topX + char_width - 0.5f;
	tempVertex[2].sy = (float)topY + height_of_char - 0.5f;
	tempVertex[2].sz = 0.0f;
	tempVertex[2].rhw = 1.0f;
	tempVertex[2].color = colour;
	tempVertex[2].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[2].tu = (float)((texU + char_width) * RecipW);
	tempVertex[2].tv = (float)((texV + height_of_char) * RecipH);

//	vb++;

	// top right
	tempVertex[3].sx = (float)topX + char_width - 0.5f;
	tempVertex[3].sy = (float)topY - 0.5f;
	tempVertex[3].sz = 0.0f;
	tempVertex[3].rhw = 1.0f;
	tempVertex[3].color = colour;
	tempVertex[3].specular = RGBALIGHT_MAKE(0,0,0,255);
	tempVertex[3].tu = (float)((texU + char_width) * RecipW);
	tempVertex[3].tv = (float)((texV) * RecipH);
